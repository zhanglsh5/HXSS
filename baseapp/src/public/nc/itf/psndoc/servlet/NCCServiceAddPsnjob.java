...
public class NCCServiceAddPsnjob extends BaseServlet {
	...
	public void doAction(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			Logger.error("同步NCC新增任职记录参数：" + param);
			System.out.println("同步NCC新增任职记录参数：" + param);
			...
			if (StringUtils.isEmpty(orgCode) || "null".equals(orgCode)) {
				Logger.error("同步NCC新增任职记录参数：参数pk_org不能为空");
			...
			if (StringUtils.isEmpty(deptCode) || "null".equals(deptCode)) {
				Logger.error("同步NCC新增任职记录参数pk_dept不能为空");
			...
			if (StringUtils.isEmpty(psncode) || "null".equals(psncode)) {
				Logger.error("同步NCC新增任职记录参数code不能为空");
			...
			if (newPsndocVO != null) {
				Logger.error("新增人员任职记录成功:" + param);

				RequestUtil.response(out, Result.ok("新增人员任职记录成功"));
				return;
			} else {
				Logger.error("新增人员任职记录失败:" + param);

				RequestUtil.response(out, Result.error("新增人员任职记录失败", param));
				return;
			}
		} catch (Exception e) {
			e.printStackTrace();
			Logger.error("同步NCC修改部门失败，返回信息：" + e.getMessage());
			RequestUtil.response(out, Result.error("新增人员任职记录失败：" + e.getMessage(), param));
			return;
		}
	}
